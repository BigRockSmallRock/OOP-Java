...
public class A04Main {
	...
	for (final CountedValue<Character> cs : array) {
	...
	for (final CountedValue<Character> cs : array) {
	    ...
	    System.out.format("%4s %,8d %7.2f%n", cs.getValue(), count, percent);
	}
	return;
    }
	...
	for (final char letter : string.toCharArray()) {
	    final CountedValue<Character> data = new CountedValue<Character>(Character.toUpperCase(letter));
	    tree.insert(data);
	}
	return;
    }
	...
	for (Integer i : testData) {
	    CountedValue<Integer> cs = new CountedValue<>(i);
	    source.insert(cs);
	}
